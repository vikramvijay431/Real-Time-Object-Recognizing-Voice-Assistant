...
def object_detector(image):
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        if classid == 0:  # person class id 
            ...
            data_list.append([class_names[classid], box[2], (box[0], box[1]-2)])
        # if you want to include more classes, simply add more elif statements here
...
while True:
    ...
    for d in data:
        if d[0] == 'person':
            ...
            engine.say(f"A person is {round(distance, 2)} inches away.")
        elif d[0] == 'cell phone':
            distance = distance_finder(focal_mobile, MOBILE_WIDTH, d[1])
            x, y = d[2]
            # Announce the detected object and distance
            engine.say(f"A cell phone is {round(distance, 2)} inches away.")
        # Other object types can be handled similarly
